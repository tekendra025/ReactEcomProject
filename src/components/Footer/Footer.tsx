          ...
          {/* footer contains section */}
           <div className="grid grid-cols-2 lg:grid-cols-4">
             <div className="space-y-4">
              ...
              <p className="text-gray-400">Your one-stop destination for trendy fashion. Quality meets style.</p>
             
             <div className="flex gap-2 lg:gap-3 justify-center lg:justify-start mb-5 lg:mb-0">
              {SocialLinks.map((item)=>(
                <a key={item.id} href={item.link} className="h-8 w-8 lg:h-9 lg:w-9 bg-gray-800 flex justify-center items-center hover:bg-primary duration-500 rounded-xl">
                  <item.icon/>
                </a>
              ))}
             </div>
             ...
             <div className="text-center lg:text-left">
                ...
                <ul className="space-y-1.5 lg:space-y-2">
                  {NavLInk.map((item)=>(
                    <li key={item.id}>
                      <a href={item.path} className="text-gray-400 hover:text-primary duration-500 text-sm lg:text-base">{item.name}</a>
                    </li>
                  ))}
                </ul>
             </div>
             ...
             <div className="text-center lg:text-left">
              <h3 className="font-bold text-base lg:text-xl mb-3 lg:mb-4">Contact Info</h3>
              <ul className="space-y-1.5 lg:space-y-2">
